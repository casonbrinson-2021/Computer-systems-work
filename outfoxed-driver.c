 ...
 * Spring 2016
 *
 * Updated Spring 2020 to detect game spoiling chicken detectives
 */
#include "outfoxed.h"
...
static bool chicken_detectives_are_shy;
static int games_where_chickens_sat_back_and_waited_for_all_clues;
...
eliminate_suspect(const Suspect *s)
{
    ...
    if (next_expected_clue == NUMBER_OF_CLUES && __builtin_popcount(remaining_suspects) == NUMBER_OF_SUSPECTS - 1) {
        chicken_detectives_are_shy = true;
        if (verbose) {
            printf("The first suspect wasn't eliminated until all clues were decoded.\n"
                   "Are the chicken detectives sleeping?\n");
        }
    }
}
...
main(int ac, char *av[])
{
    ...
    if (verbose) {
        ...
        for (int i = 0; i < N_GAMES; i++) {
            chicken_detectives_are_shy = false;
            play_one();
            if (chicken_detectives_are_shy)
                games_where_chickens_sat_back_and_waited_for_all_clues++;
        }
        if (no_share_count == N_GAMES) {
            printf("Chicken detectives are not sharing work\n");
            abort();
        }
        if (games_where_chickens_sat_back_and_waited_for_all_clues == N_GAMES && N_GAMES > 10) {
            /* Although theoretically possible, it's highly unlikely for there to
             * be many games where the chicken detectives couldn't eliminate any suspect
             * until 12 clues have been decoded.  Something's up with them chickens.
             * Fail it.
             */
            printf("Chicken detectives seem to be waiting before taking part in game\n");
            abort();
        }
    }
